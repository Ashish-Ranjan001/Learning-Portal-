...
//   isVideoCompleted = false
//   videoWatchTime = 0
//   minimumWatchTime = 10 // Minimum 10 seconds watch time
//   showVideoOverlay = true // Controls the play button overlay
//   actualVideoWatchTime = 0 // Track actual watch time
//   videoCheckInterval: Subscription | null = null
...
//       this.videoCheckInterval.unsubscribe()
//     }
//     this.removeQuizEventListeners()
//   }
...
//   // IMPROVED video completion check
//   private checkVideoCompletion(): void {
//     if (this.isVideoCompleted) return;

//     const hasVideo = this.selectedModule?.videoUrl || this.currentVideoData?.videopath;
//     if (!hasVideo) return;

//     // For short videos (less than 60 seconds), adjust completion logic
//     if (this.videoDuration > 0 && this.videoDuration <= 60) {
//       // For videos 60 seconds or less, check if watched to the end OR minimum time met
//       const watchedToEnd = this.videoProgress >= 95; // Allow 95% for short videos
//       const minimumTimeMet = this.actualVideoWatchTime >= Math.min(this.minimumWatchTime, this.videoDuration * 0.8);
      
//       if (watchedToEnd || minimumTimeMet) {
//         this.onVideoCompleted();
//       }
//     } else {
//       // For longer videos, use standard logic
//       const progressMet = this.videoProgress >= (this.videoCompletionThreshold * 100);
//       const timeMet = this.actualVideoWatchTime >= this.minimumWatchTime;
      
//       if (progressMet && timeMet) {
//         this.onVideoCompleted();
//       }
//     }
//   }

//   // UPDATED: Improved video tracking without simulation
//   startWatchingVideo(): void {
//     if (!this.videoStarted) {
//       this.videoStarted = true
//       this.showVideoOverlay = false // Remove the play button overlay
...
//       this.videoCheckInterval = interval(100).subscribe(() => {
//         // Only increment if we have actual video progress
//         if (this.videoProgress > 0) {
//           this.actualVideoWatchTime += 0.1;
//         }
//       });

//       // Set up simulated duration for videos where we can't get actual duration
//       if (this.videoDuration === 0) {
//         // Try to estimate duration based on module info or set a default
//         setTimeout(() => {
//           if (this.videoDuration === 0) {
//             // If still no duration after 2 seconds, assume it's a short video
//             this.videoDuration = 60; // Default to 60 seconds
//           }
//         }, 2000);
//       }

//       // Start a fallback timer for video completion
//       this.videoProgressTimer = interval(500).subscribe(() => {
//         this.checkVideoCompletion();
//       });
//     }
//   }
...
//     console.log("✅ Video completed! Progress:", this.videoProgress.toFixed(1) + "%")
//     console.log("✅ Watch time:", this.actualVideoWatchTime.toFixed(1) + " seconds")
...
//   onVideoLoad(): void {
//     console.log("Video loaded successfully")
//     // Start checking for video completion immediately for short videos
//     if (this.videoDuration > 0 && this.videoDuration <= 60) {
//       this.startWatchingVideo();
//     }
//   }
...
//   // UPDATED: More flexible completion check
//   canCompleteModule(): boolean {
//     if (!this.selectedModule) return false
...
//     if (hasVideo) {
//       // For video modules, check if video is watched
...
//       return "✅ Video Completed - Ready to mark as complete"
//     } else if (this.videoStarted) {
//       return `📺 Watching... ${this.videoProgress.toFixed(1)}% completed (${this.actualVideoWatchTime.toFixed(0)}s watched)`
//     } else {
//       return "▶️ Click play to start watching"
//     }
//   }
...
//           return `Watch the video completely to mark as complete`
//         } else if (this.videoProgress < this.videoCompletionThreshold * 100) {
//           return `Watch ${Math.ceil(this.videoCompletionThreshold * 100 - this.videoProgress)}% more of the video`
//         } else if (this.actualVideoWatchTime < this.minimumWatchTime) {
//           return `Watch for ${Math.ceil(this.minimumWatchTime - this.actualVideoWatchTime)} more seconds`
//         }
//       }
//       return "Complete the required activities first"
//     }
